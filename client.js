...
function replaceIdeaName(idea) {
	...
	return $.trim('<a class="ideaname suggname" href="#?q=$1">'+idea.substr(0,te)+'</a>'+idea.substr(te));
}

function replaceIdeaName(idea,pid) {
	var te=findTitleEnd(idea);
    return $.trim('<a class="ideaname suggname" name="'+pid+'" href="">'+idea.substr(0,te)+'</a>'+idea.substr(te)); //$$$ replaced index.1.7_suggestionbox_inProgress.php#?post='+pid+'
}
