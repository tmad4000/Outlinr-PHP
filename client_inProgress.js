...
function entryNodeToHTML(entryNode) {
			...
			if(entryNode.pid!=null) {
				...
				status ="<td class='status'>" + '<a href="index.1.7_mitsuggestionbox_inProgress.php" rel="popover" data-content="'+statusTable[entryNode.status]+'" data-original-title=""><div class="status sc'+entryNode.status +'" >'+ '</div></a>' + "</td>";
...
function replaceIdeaName(idea,pid) {
    ...
    nameEnd = Math.min(50,i);
    return $.trim('<a class="ideaname" name="'+pid+'" href="index.1.7_mitsuggestionbox_inProgress.php#?post='+pid+'">'+idea.substr(0,nameEnd)+'</a>'+idea.substr(nameEnd));
}
